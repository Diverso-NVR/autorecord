...
from .apis.nvr_api import send_record
...
class RecordHandler:
    ...
    def start_record(self, room: Room) -> None:
        ...
        sound = subprocess.Popen(
            "ffmpeg -use_wallclock_as_timestamps true -rtsp_transport tcp -i rtsp://"
            + room.sound_source
            + " -y -c:a copy -vn -f mp4 "
            + HOME
            + "/vids/sound_"
        ...
        for source in room.sources:
            ...
            process = subprocess.Popen(
                "ffmpeg -use_wallclock_as_timestamps true -rtsp_transport tcp -i "
                + source.rtsp
                + " -y -c:v copy -an -f mp4 "
                + HOME
                + "/vids/vid_"
    ...
    def config(self, room: Room) -> None:
        ...
        hour = (
            "0" + str(current_time.hour)
            if current_time.hour < 10
            else str(current_time.hour)
        )
        minute = (
            "0" + str(current_time.minute)
            if current_time.minute < 10
            else str(current_time.minute)
        )

        record = f"{today.year}-{month}-{day}_{hour}:{minute}_{room.name}_"
        self.record_names[room.id] = record
        self.state[record] = {"datetime": current_date, "room_name": room.name}
    ...
    async def prepare_records_and_upload(self, room: Room) -> None:
        ...
        for folder_id, folder_parent_ids in folders.items():
            ...
            time_folder_url = await create_folder(time, date_folder_url.split("/")[-1])

        await self.sync_and_upload(
            record_name, room.sources, time_folder_url.split("/")[-1]
        )

    async def sync_and_upload(
        self, record_name: str, room_sources: list, folder_id: str
    ) -> None:
        logger.info(
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=len(room_sources)
            ) as pool:
                await asyncio.gather(
                    *[
                        self.async_add_sound(pool, source, record_name)
            ...
            await asyncio.gather(
                *[
                    self.uploader(record_name, source, folder_id)
                    for source in room_sources
                ]
            )
        finally:
        ...
        try:
            proc = subprocess.Popen(
                [
                    "ffmpeg",
                    "-i",
                    HOME + "/vids/sound_" + record_name + ".aac",
                    "-i",
                    HOME + "/vids/vid_" + record_name + source_id + ".mp4",
                    "-y",
                    "-shortest",
                    "-c",
                    "copy",
    ...
    async def uploader(self, record_name, source, folder_id):
        try:
            file_name = record_name + source.ip.split(".")[-1] + ".mp4"
            logger.info(f"Uploading {HOME}/vids/{file_name}")

            record_data = self.previous_state[record_name]
            room_name = record_data["room_name"]
            record_datetime = record_data["datetime"]

            file_id = await upload(f"{HOME}/vids/{file_name}", folder_id)
            await send_record(
                room_name=room_name,
            ...
            logger.error(f"Failed to upload file {file_name}", exc_info=True)
        finally:
